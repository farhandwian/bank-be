	...
	return func(c fiber.Ctx) error {
		...
		* ----------------------------------*/
		...
		if authHeader == "" {
			...
			return c.Status(http.StatusUnauthorized).JSON(shared.StandardResponse{
...
func RoleBasedMiddleware(allowedRoles ...string) fiber.Handler {
	return func(c fiber.Ctx) error {
		var (
			lvState1       = shared.LogEventStateValidateToken
		...
		* ----------------------------------*/
		...
		if !ok {
			...
			return c.Status(http.StatusForbidden).JSON(shared.StandardResponse{
